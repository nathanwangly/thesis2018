    ...
    while instructions_mode:
        for event in pygame.event.get():
            ...
            # Instructions 5 - GROUP 2
                ...
                instruction_string2 = 'You will receive information about the first group of participants.'

                textPrint(instruction_string1, (display_width / 2, display_height / 2.5), bold=True)
                nextLine(instruction_string2)

            # Instructions 6 - NORM INFORMATION
            elif instruction_stage == 6:
                displaysurf.fill(white)
                instruction_string1 = 'Each month, you will receive feedback like the message below.'
                instruction_string2 = '30% of previous participants had saved more than you by this month.'
                instruction_string3 = 'These are social norm messages, which show you how other participants have played the game.'
                instruction_string4 = 'These messages are being provided to encourage you to save more money in the game.'

                textPrint(instruction_string1, (display_width / 2, display_height / 3))

                nextLine(instruction_string2, bold = True, colour=red)
                nextLine(instruction_string3)
                nextLine(instruction_string4)
            ...
            # Instructions 7 - COMPREHENSION CHECK
            elif instruction_stage == 7:
...
def game_round():
    ...
    while game_round:
        ...
        # Instructions
        instruction_string1 = 'You have saved another $' + str(
            income) + ' this month. Your savings account balance is now $' + str(current_savings) + '.'
        ...
        textPrint('I would like to spend:', (display_width / 2.4, display_height / 1.7), size=font_size_large,
                  bold=True)
        ...
        for event in pygame.event.get():
            ...
            input_box_cover = pygame.Rect((display_width + 4) / 2, (display_height + 4) / 1.8, input_box_width - 4,
                                          input_box_height - 4)
            ...
            textLeftAlign(remaining_balance_string, size=font_size_large, bold=True,
                          top_left=(display_width / 2.82, display_height / 1.45))

            # Cover remaining balance text
            remaining_balance_cover = pygame.Rect(textLeftAlign.left, textLeftAlign.top, textLeftAlign.width + 100,
                                                  textLeftAlign.height)
...
def financial_emergency():
    ...
    textPrint('FINANCIAL EMERGENCY!', (display_width / 2, display_height / 4), bold=True, size=font_size_extralarge,
              colour=red)
    ...
    textPrint('Oh no! There was a severe storm and your house was damaged due to a flood.',
              (display_width / 2, display_height / 1.4))
...
def financial_emergency_calculation():
    ...
    textPrint('The repair bill comes to a total of $' + str(repair_cost) + '.', (display_width / 2, display_height / 4),
              size=font_size_extralarge)

    textRightAlign('Your account balance:', size=font_size_large, bold=True,
                   top_right=(display_width / 2, display_height / 2))
    textRightAlign('Repair cost:', size=font_size_large, bold=True, colour=red,
                   top_right=(display_width / 2, display_height / 1.7))
    textRightAlign('Your remaining balance:', size=font_size_large, bold=True,
                   top_right=(display_width / 2, display_height / 1.35))

    account_savings_string = '$' + str(current_savings)
    textLeftAlign(account_savings_string, size=font_size_large, bold=True,
                  top_left=(display_width / 1.8, display_height / 2))

    repair_cost_string = '$' + str(repair_cost)
    textLeftAlign(repair_cost_string, size=font_size_large, bold=True, colour=red,
                  top_left=(display_width / 1.8, display_height / 1.7))

    pygame.draw.line(displaysurf, black, (display_width / 5, display_height / 1.5),
                     (display_width * 4 / 5, display_height / 1.5), 5)
    ...
    textLeftAlign(remaining_balance_string, size=font_size_large, bold=True, colour=text_colour,
                  top_left=(display_width / 1.8, display_height / 1.35))
...
def experiment_end():
    ...
    textPrint('Thank you for participating in this experiment :)', (display_width / 2, display_height / 2),
...
def nextLine(text, bold=False, size=font_default_size, colour=black):
    ...
    textPrint(text, centre=(textPrint.centre[0], textPrint.centre[1] + line_spacing), bold=bold, size=size,
              colour=colour)
